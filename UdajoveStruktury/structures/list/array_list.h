...
namespace structures
{
	...
	inline bool ArrayList<T>::equals(Structure& other)
	{
		if (this == &other) return true;
			...
			if (otherArrayList != nullptr) {
				if (size_ != otherArrayList->size()) return false;
					else {
						for (size_t i = 0; i < size_; i++) {
							if (array_->at(i) != otherArrayList->array_->at(i))return false;
						}
						return true;
					}
			}
	...
	{
		Array<T>* biggerArray_ = new Array<T>(size_ * 2);
		Array<T>::copy(*array_,0,*biggerArray_,0,size_);
		delete array_;
		array_ = biggerArray_;
	}
	...
	inline Iterator<T>& ArrayList<T>::ArrayListIterator::operator=(Iterator<T>& other)
	{
		if (this != &other)
		{
			auto otherIt = dynamic_cast<ArrayListIterator&>(other);
	...
	{
		auto otherIt = dynamic_cast<ArrayListIterator*>(&other);
		return otherIt == nullptr || this->arrayList_ != otherIt->arrayList_ || this->position_ != otherIt->position_;
	}
