...
namespace structures
{
	...
	inline T& SequenceTable<K, T>::find(const K& key)
	{
		...
		if (item != nullptr) return item->accessData();
		else throw std::out_of_range("No such key in the table!");
	}

	template<typename K, typename T>
	inline void SequenceTable<K, T>::insert(const K& key, const T& data)
	{
		if (!containsKey(key)) list_->add(new TableItem<K, T>(key, data));
	...
	inline T SequenceTable<K, T>::remove(const K& key)
	{
		...
		else throw std::logic_error("No such Key exists!");
	}
