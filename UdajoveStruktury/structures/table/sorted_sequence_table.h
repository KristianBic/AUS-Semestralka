...
namespace structures
{
	...
	inline void SortedSequenceTable<K, T>::insert(const K& key, const T& data)
	{
		...
		if (!found) this->list_->insert(new TableItem<K, T>(key, data), index);
	...
	inline int SortedSequenceTable<K, T>::indexOfKey(K key, int indexStart, int indexEnd, bool& found)
	{
		...
		K midkey = this->list_->at(mid)->getKey();
		...
		if (midkey == key) {
			...
			found = false;
			return key < midkey ? mid : mid + 1;
		}
		else {
			return key < midkey ? indexOfKey(key, indexStart, mid, found) : indexOfKey(key, mid + 1, indexEnd, found);
			//if (key < midkey) return this->indexOfKey(key, indexStart, mid, found);
			//else return this->indexOfKey(key, mid + 1, indexEnd, found);
		}
	}
}